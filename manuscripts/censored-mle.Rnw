% Time-stamp: <2017-08-24 18:57:18 (slane)>
...
Invasive marine non indigenous species (NIS) are a major threat to biodiversity leading to dramatic shifts in population, community and ecosystem dynamics, and many have serious environmental and economic implications \citep{Carlton1993-cc,Ruiz1997-ji,Vitousek1996-zr}. Vessels are the single most important vector for the introduction and spread of marine NIS globally \citep{Molnar2008-dm}, effectively removing natural barriers to dispersal, including ocean currents and distance \citep{Paini2012-md,Ruiz1997-ji}. For example, the annual cost (losses and control costs) to the United States resulting from the introduction of the Zebra mussel (species name), Asian clam (species name) and Shipworm (species name)\footnote{Species names to be added.} is estimated at over USD\$3bn\citep{Pimentel2011-pe}.
...
All analyses were performed using R version 3.4.0 \citep{R_Core_Team2017-zb}; the multilevel models were fit using the RStan interface to stan \citep{Stan_Development_Team2016-xb}, and multiple imputation was performed use the R package mice \citep{Van_Buuren2011-ne}.
