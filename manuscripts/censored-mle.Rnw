% Time-stamp: <2017-05-03 13:42:09 (slane)>
...
All analyses were performed using R version 3.3.3 \cite{R_Core_Team2017-zb}; the multilevel models were fit using the RStan interface to stan \cite{Stan_Development_Team2016-xb}, and multiple imputation was performed use the R package mice \cite{Van_Buuren2011-ne}.
...
Figure XXX displays the observed and imputed vessel-level data for both the number of days since the vessel was last used, and the median number of trips by the vessel. Figure YYY displays the observed and imputed vessel-level type of paint.
