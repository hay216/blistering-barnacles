% Time-stamp: <2017-05-04 13:56:41 (slane)>
...
<<imp-cont,fig.cap="Comparison of observed and imputed data for the number of days since the vessel was last used, and the median number of trips by the vessel. The leftmost boxplot in each figure is the observed distribution, all others include the imputed values.",fig.subcap=c("Number of days since the vessel was last used.", "Median number of trips by the vessel."),out.width="45%">>=
...
<<imp-cat,fig.cap="Comparison of observed and imputed data for anti-fouling paint type. A random selection from the imputed datasets are shown. The top-left panel (labelled 0) shows the observed distribution.",out.width="45%">>=
set.seed(76)
...
Figure XXX displays the median and 80\% intervals of the estimated vessel-level intercepts, with respect to the four ordinal predictors: the number of days since the vessel was last used; the number of days since the vessel was last cleaned; the median number of trips undertaken by the vessel; and the vessel's hull surface area. Figure XXXa shows the vessel-level intercepts coloured by vessel type, whilst Figure XXXb shows the vessel-level intercepts coloured by anti-fouling paint type. Shown in each panel are the estimated (median) regression lines from M1.

Figures XXXa and XXXb are suggestive of interactions between the number of days since the vessel was last cleaned and the vessel type, the median number of trips undertaken by the vessel and the vessel type, and the median number of trips taken by the vessel and the anti-fouling paint type. There does not appear to be any relationship between the vessel's hull surface area and the vessel-level intercepts. These observations led to the development of M2 (removing the hull surface area predictor) and M3 (including the possible interaction terms).

\subsubsection{Reduction to final model form}
\label{sec:reduct-final-model}

Figure YYY displays the median and 80\% intervals of the estimated vessel-level intercepts, with respect to the two ordinal predictors remaining in M3: the number of days since the vessel was last used and the median number of trips undertaken by the vessel. Figures YYYa and YYYb show the vessel-level intercepts coloured by vessel type and Figure YYYc shows the vessel-level intercepts coloured by anti-fouling paint type. Shown in each panel are the estimated (median) regression lines from M3.

Figure YYYa is suggestive of an interaction between the number of days since the vessel was last cleaned and the vessel type, however Figures YYYb and YYYc do not show any evidence of interactions between the median number of trips undertaken by the vessel and the vessel type or the anti-fouling paint type. These observations led to the development of M4 as the most likely model.

Figure ZZZ shows the regression estimates from M4, in relation to the estimated vessel-level intercepts. Figure ZZZa shows the number of days since the vessel was last used and the vessel-type estimated interaction, and Figure ZZZb shows the estimated effect of the number of days since the vessel was last cleaned (adjusted by vessel-type).
