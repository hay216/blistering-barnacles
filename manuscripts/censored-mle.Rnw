% Time-stamp: <2017-05-11 17:40:09 (slane)>
...
Table~\ref{tab:looic} compares the models by their leave-one-out information criterion (LOOIC, analogous to AIC, see \cite{Vehtari2016-dp}). \ref{eq:M4} is the best-performing model based on LOOIC, but not significantly so.
